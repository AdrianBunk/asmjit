...
const armdata = readJSON(path.join(__dirname, "..", "db", asmdb.arm.dbName));
const armisa = new asmdb.arm.ISA(armdata);
...
class Generator {
  ...
  genFunc() {
    ...
    function formatRecords(records, fn, n) {
      let s = "";
    ...
    let table =
      "static const InstDispatchRecord instDispatchTable[] = {\n" +
        indent(formatRecords(dispatchTable, (record) => { return `{${record.group}, ${record.index}}`; }, 10), 2) +
      "\n};\n";
